...
func (ev *FSAEnv) StepFSA() {
	...
	// --- Reward only on critical "hard" transitions ---
	// In the hard FSA, only deliver reward when the current state is C (5 or 6)
	// and the next output is H (7) or I (8)
	isHard := ev.FSAHard && (ev.StateNode == 5 || ev.StateNode == 6)
	nextOut := ev.FSAOuts[ev.NextStateNode]
	if isHard && (nextOut == 7 || nextOut == 8) {
		ev.Reward.Values[0] = float64(ev.RewVal)
	} else {
		ev.Reward.Values[0] = float64(ev.NoRewVal)
	}
